...
import LevelCongratulations from "./distjs/LevelCongratulations";
...
export default {
  ...
  data() {
    return {
      ...
      showradar: false,
      point_arry: [[0.1, 1], [0.1, 1], [0.1, 1], [0.1, 1]],
      dialogShow0: false
    };
  },
  computed: {
  ...
  mounted: function() {
    ...
    self.defaultScore =
      100 - Math.ceil(self.LowScore.length / self.ScoreAll.length * 100) + "%";
    ...
    [
      self.scorePartListen,
      self.scorePartReading,
      self.scorePartSentences,
      self.scorePartGrammar
    ].map((item, key) => {
